...
export default function Dashboard() {
  const { solicitudes, fetchSolicitudes } = useCrmStore()

  useEffect(() => { fetchSolicitudes() }, [])

  const stats = useMemo(() => {
    const activos = solicitudes.filter(s => s.estado === 'activo')
    const mora    = solicitudes.filter(s => s.estado === 'mora')
    const pagados = solicitudes.filter(s => s.estado === 'pagado')
    const cartera = activos.reduce((s, x) => s + (x.totalPagar || 0), 0)
    return { total: solicitudes.length, activos: activos.length, mora: mora.length, pagados: pagados.length, cartera }
  }, [solicitudes])
